...
//건물이 날 수 있느 기능 구현. 건물의 일반적인 기능이 아님.
interface Fly
{
	void flyBuilding();
}
...
class Barracks extends Building implements Fly
{
	...
	//모든 건물이 낳면 안되므로 인터페이스로 나는 기능 제공
...
class Factory extends Building implements Fly 
{
	...
	//모든 건물이 날면 안되므로 인터 페이스로 나느 기능 제공
	public void flyBuilding()
	{
		System.out.println("건물이 날아서 이동하게 합니다.");
	}
...
class Bunker extends Building
{
	void doBuild() 
	{
		System.out.println("기갑형 유닛이 숨은 건물을 짓습니다.");
	}
	void doDefense() 
	{
		System.out.println("숨은 유닛을 적의 공격으로 부터 보호합니다.");
	}
}
